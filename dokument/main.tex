...
V tejto práci riešime problematiku kybernetickej bezpečnosti pomocou využitia ontológií. Popíšeme si, čo vôbec ontológia je, aké sú jej výhody a využitia. Ukážeme si základný model, z ktorého vychádzame, a ktorý rozdeľuje celú doménu do viacerých častí a predstavíme si niektoré existujúce riešenia. Na základe analýzy si určíme najlepšie ontologické riešenie a takto vybratú ontológiu rozšírime o niektoré časti, ktoré podľa daného modelu nespĺňa. Tieto časti popíšeme v našej vlatsnej ontológii podľa požiadaviek modelu a štandardov, ktoré na danú problematiku existujú, ale ešte nemajú ontologické riešenie. Pomocou nášho návrhu následne zmapujeme existujúce dáta a prepojíme ich.
\\
\\
...
\assignment{MH: $\uparrow$ Pozor, toto nema byt nejaka vseobecna rozpravka. To ze kyberneticke utoky su realna hrozba to predsa kazdy vie, to mozes vynechat. Bolo by lepsie zacat od toho, ze aby sa im dalo lepsie zabranovat, organizacie publikuju data o utorkoch, zranitelnostiach, atd. Napisat nejaky sumar o tychto standardoch, aj s citaciami. Uplne tu opat chyba CTI model, na ktory my v praci referujeme...\\
MR: zmenena prva cast, ten uvod by som nechal, vnesiem citatela do toho ze sa pojednava o utokoch na ktory nasleduju dalsie vety. lepsi uvod ma nenapada, lebo inak by som to musel priamo pisat\\
MR: CTI model som spomenul v odseku nizsie}
\overview{Problem}
...
\assignment{MH: $\uparrow$ Toto je prilis zjednodusene, nie je to s tymi standardmi az take zle, niekto kto sa v nich vyzna by mohol namietat. Keby sa ti to podarilo zlepsit, tak potom toto $\downarrow$ premostenie na ontologie je celkom fajn, ale dalo by sa este malo zlepsit (najma: dorozumiet sme sa uz so snahou vedeli aj predtym)\\
MR: upravene}
...
\assignment{MH $\uparrow$ Este tie data, dost sa tu tym ohanas, ze si ich importoval,
ale ich komplexne naimportovanie a zhromazdenie a publikovanie je future work stefanovych
studentov, nie? Mozno to chcel len malinko inak podat/upresnit... Ty predsa len nejake data
s prilohami odovzdas, ano?\\
MR: ano odovzdam ukazku dat, nechal som tu iba mapovanie, import som vyhodil, to je pravda, to ja riesit nebudem}
